---
title: "Face-Names Pairs Test (in Person)"
...
dat <- read_csv("face_names_in_person.csv")
...
fit3 <- glmer(cbind(Score, total - Score) ~ Trial + FN +
                (Trial || FN:Ps),
              family = binomial,
              data = dat)
...
dat_pred_ind <- expand.grid(Trial = seq(1, 4, length = 200),
                            FN = levels(dat$FN),
                            Ps = levels(dat$Ps)) %>%
  filter(!(FN == "massed" & Ps %in% paste(58:61)))
...
#ggsave("spaced_massed_nodiff.png", dpi = 800, units = "in", w = 8, h = 5)
```
...
```{r}
# to combine the subparts as one figure
# library(gridExtra)
# png("fig2.png", w = 12, h = 8, res = 800, units = "in")
# grid.arrange(fig1_a, fig1_b, fig1_c, fig1_d, ncol = 2)
# dev.off()
```